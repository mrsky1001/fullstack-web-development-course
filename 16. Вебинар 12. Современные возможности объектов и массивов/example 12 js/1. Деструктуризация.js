...
const obj1 = numbers[0]
const obj2 = numbers[1]
const obj3 = numbers[3]

// Базовая деструктуризация
const [first, second, third] = numbers;
console.log('Базовая деструктуризация:', first, second, third); // 1 2. Добавляем только html + css 3
...
console.log('Rest оператор:', head, tail); // 1 [2. Добавляем только html + css, 3, 4, 5]

// Значения по умолчанию
const [a = 10, b = 20, c = 30] = [1, 2];
console.log('Значения по умолчанию:', a, b, c); // 1 2. Добавляем только html + css 30
...
[x, y] = [y, x];
console.log('Обмен переменных:', x, y); // 2. Добавляем только html + css 1

// ДЕСТРУКТУРИЗАЦИЯ ОБЪЕКТОВ
const personWorkerJobDistant = {
    name: 'Alice',
    age: 30,
    city: 'Paris',
    country: 'France'
...
const { name, age } = personWorkerJobDistant;
...
//     const { name, age } = personWorkerJobDistant;
//
//     console.log(personWorkerJobDistant.name)
//     let ageNew = personWorkerJobDistant.age + 12
//
// }
...
const { name: personName, city: residence } = personWorkerJobDistant;
