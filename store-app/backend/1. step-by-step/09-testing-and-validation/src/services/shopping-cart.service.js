/**
 * SERVICE: Shopping Cart (Финальная версия)
 */
...
exports.findAllProducts = async (userId) => {
    try {
        const sql = `
            SELECT s.item_id, s.user_id, s.product_id, s.item_quantity,
                   p.product_name, p.product_price, p.product_category, p.product_img
...
exports.insertRow = async (row) => {
    try {
        ...
        const newItem = { id: mockCart.length + 1, ...row };
...
exports.updateQuantityRow = async (row, userId) => {
    try {
        ...
        const index = mockCart.findIndex(item => item.id === row.id && item.userId === userId);
        if (index !== -1) {
...
exports.deleteRow = async (rowId, userId) => {
    try {
        ...
        const index = mockCart.findIndex(item => item.id === rowId && item.userId === userId);
        if (index !== -1) {
