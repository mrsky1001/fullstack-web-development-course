...
//offsetWidth — свойство в JavaScript, которое возвращает полную ширину элемента (в пикселях).
...
// const minPriceValueStr = minPriceEl.value
// const minPriceValueNum = parseFloat(minPriceValueStr) // 12 или NaN
//
// let minPrice2 = 0;
//
// if (minPriceValueNum){
//     minPrice2 = minPriceValueNum
// }else {
//     minPrice2 = 0
// }
//
// const minPrice = minPriceValueNum || 0 || undefined || 2;
...
function filterProducts() {
    ...
    //Infinity в JavaScript — глобальное свойство, которое представляет бесконечное число. Это число больше любого другого числа.
    ...
    // Свойство .dataset в HTML и JavaScript (JS) — это способ хранения пользовательских данных непосредственно в HTML-разметке и доступа к ним из JavaScript.
    // Проверка каждой карточки продукта на соответствие фильтрам
    productCards.forEach(card => {
        const price = parseFloat(card.dataset.price);
        const name = card.dataset.name.toLowerCase();
        const specs = card.dataset.specs.split(', '); // Мощность: 500 Вт, Скорость: 3000 об/мин, Глубина реза: 55 мм, Вес: 1.5 к
        ...
        const matchesName = name.includes(nameFilter);// true or false
        const matchesPrice = price >= minPrice && price <= maxPrice; // true or false
        const matchesPower = power >= minPower && power <= maxPower;// true or false
        const matchesSpeed = speed >= minSpeed && speed <= maxSpeed;// true or false
        const matchesCutDepth = cutDepth >= minCutDepth && cutDepth <= maxCutDepth;// true or false
        const matchesWeight = weight >= minWeight && weight <= maxWeight;// true or false

        // Показать или скрыть карточку в зависимости от результата фильтрации
        card.style.display = matchesName && matchesPrice && matchesPower && matchesSpeed && matchesCutDepth && matchesWeight ? 'flex' : 'none';

        // card.style.display = "flex";
        //
        // if (matchesName && matchesPrice && matchesPower && matchesSpeed && matchesCutDepth && matchesWeight){
        //     card.style.display = "flex";
        // } else   {
