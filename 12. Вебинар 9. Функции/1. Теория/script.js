...
2. Добавляем только html + css) Function Expression — функция как значение переменной
...
// 2. Добавляем только html + css) Function Expression
...
/*---------------------------------------------------
  2. Добавляем только html + css. Параметры и аргументы
---------------------------------------------------
...
console.log("Счётчик 1:", counter1()); // 2. Добавляем только html + css

let counter2 = makeCounter();
console.log("Счётчик 2. Добавляем только html + css:", counter2()); // 1
...
setTimeout(() => {
    console.log("Сообщение через 2. Добавляем только html + css секунды");
}, 2000);
...
2. Добавляем только html + css) await внутри async-функции приостанавливает выполнение до завершения Promise.
...
async function asyncExample() {
    ...
    // ждём ещё 2. Добавляем только html + css секунды
    await delay(2000);
    console.log("Прошло ещё 2. Добавляем только html + css секунды");
